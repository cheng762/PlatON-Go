...
// Deprecated: Use WithdrewDelegation.
// Obsolete from version 0.16.0
	...
	if can.IsNotEmpty() && stakingBlockNum == can.StakingBlockNum {
		if can.IsValid() {
			...
			if gov.Gte0150VersionState(state) {
				...
				log.Error("set can power fail", "blockNumber", blockNumber, "delAddr", delAddr, "can", nodeId, "stakingBlockNum", stakingBlockNum, "realSub", realSub)
			}
		}
